...
module.exports = (sequelize) => {
  const Resident = sequelize.define('Resident', {
    ...
    relationship_to_head: {
      type: DataTypes.ENUM('head', 'spouse', 'child', 'parent', 'sibling', 'grandparent', 'grandchild', 'other_relative', 'tenant', 'servant', 'other'),
      allowNull: false,
      defaultValue: 'other'
    },
    phone_number: {
    ...
    education_level: {
      type: DataTypes.ENUM('illiterate', 'primary', 'secondary', 'higher_secondary', 'graduate', 'post_graduate', 'professional', 'other'),
      allowNull: true
    },
    monthly_income: {
    ...
    vaccination_status: {
      ...
      allowNull: true,
      defaultValue: {},
      comment: 'COVID-19 and other vaccination information'
    },
    health_conditions: {
      type: DataTypes.JSONB,
      allowNull: true,
    ...
    anniversary_date: {
      type: DataTypes.DATEONLY,
      allowNull: true,
      comment: 'Wedding anniversary date'
    },
    notes: {
      type: DataTypes.TEXT,
      allowNull: true
  ...
  Resident.prototype.getUpcomingAnniversary = function() {
    ...
    let anniversary = new Date(thisYear, anniversaryDate.getMonth(), anniversaryDate.getDate());
  ...
  Resident.findAnniversaries = function(startDate, endDate) {
    ...
    return this.findAll({
      where: {
        ...
        [Op.or]: [
          sequelize.where(
            sequelize.fn('EXTRACT', sequelize.literal('MONTH FROM anniversary_date')),
            { [Op.between]: [startMonth, endMonth] }
          ),
          sequelize.where(
            sequelize.fn('EXTRACT', sequelize.literal('DAY FROM anniversary_date')),
            { [Op.between]: [startDay, endDay] }
          )
        ]
      },
