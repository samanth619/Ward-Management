...
module.exports = (sequelize) => {
  const ResidentBankDetails = sequelize.define('ResidentBankDetails', {
    id: {
      type: DataTypes.UUID,
    ...
    account_type: {
      type: DataTypes.ENUM('savings', 'current', 'fixed_deposit', 'recurring_deposit', 'overdraft'),
      allowNull: true,
    ...
    verification_method: {
      type: DataTypes.ENUM('penny_drop', 'passbook', 'cheque', 'statement', 'manual'),
      allowNull: true
    },
    is_primary_account: {
      type: DataTypes.BOOLEAN,
    ...
    joint_account_holders: {
      type: DataTypes.TEXT,
      allowNull: true,
      comment: 'JSON array of joint account holder names'
    },
    upi_id: {
      ...
      beforeCreate: async (bankDetails, options) => {
        ...
        if (sequelize.models.AuditTrail) {
          await sequelize.models.AuditTrail.create({
            table_name: 'resident_bank_details',
            record_id: bankDetails.id,
            action: 'CREATE',
            changes: JSON.stringify(bankDetails.dataValues),
            user_id: bankDetails.created_by,
      ...
      beforeUpdate: async (bankDetails, options) => {
        if (sequelize.models.AuditTrail && bankDetails.changed()) {
          await sequelize.models.AuditTrail.create({
            ...
            record_id: bankDetails.id,
            action: 'UPDATE',
            changes: JSON.stringify(bankDetails.changed()),
            previous_values: JSON.stringify(bankDetails._previousDataValues),
            new_values: JSON.stringify(bankDetails.dataValues),
            user_id: bankDetails.updated_by,
            ip_address: options.ip_address,
            user_agent: options.user_agent
          });
        }
      ...
      beforeDestroy: async (bankDetails, options) => {
        if (sequelize.models.AuditTrail) {
          await sequelize.models.AuditTrail.create({
            ...
            record_id: bankDetails.id,
            action: 'DELETE',
            previous_values: JSON.stringify(bankDetails.dataValues),
