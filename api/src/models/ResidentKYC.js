...
module.exports = (sequelize) => {
  const ResidentKYC = sequelize.define('ResidentKYC', {
    id: {
      type: DataTypes.UUID,
    ...
    vehicle_classes: {
      type: DataTypes.STRING(100),
      allowNull: true,
      comment: 'Authorized vehicle classes (LMV, HMV, etc.)'
    },
    // Passport Details
    passport_number: {
      type: DataTypes.STRING(20),
    ...
    ration_card_type: {
      ...
      allowNull: true,
      comment: 'APL: Above Poverty Line, BPL: Below Poverty Line, AAY: Antyodaya Anna Yojana, PHH: Priority Household'
    },
    ...
    ayushman_bharat_number: {
      type: DataTypes.STRING(20),
      allowNull: true,
      comment: 'PM-JAY Ayushman Bharat card number'
    },
    ...
    other_health_insurance: {
      type: DataTypes.TEXT,
      allowNull: true,
      comment: 'Other health insurance details in JSON format'
    },
    // Educational Certificates
    education_certificates: {
      type: DataTypes.TEXT,
      allowNull: true,
      comment: 'Educational certificate details in JSON format'
    },
      ...
      beforeCreate: async (kyc, options) => {
        ...
        if (sequelize.models.AuditTrail) {
          await sequelize.models.AuditTrail.create({
            table_name: 'resident_kyc',
            record_id: kyc.id,
            action: 'CREATE',
            changes: JSON.stringify(kyc.dataValues),
            user_id: kyc.created_by,
      ...
      beforeUpdate: async (kyc, options) => {
        ...
        if (sequelize.models.AuditTrail && kyc.changed()) {
          await sequelize.models.AuditTrail.create({
            ...
            record_id: kyc.id,
            action: 'UPDATE',
            changes: JSON.stringify(kyc.changed()),
            previous_values: JSON.stringify(kyc._previousDataValues),
            new_values: JSON.stringify(kyc.dataValues),
            user_id: kyc.updated_by,
            ip_address: options.ip_address,
            user_agent: options.user_agent
          });
        }
      ...
      beforeDestroy: async (kyc, options) => {
        if (sequelize.models.AuditTrail) {
          await sequelize.models.AuditTrail.create({
            ...
            record_id: kyc.id,
            action: 'DELETE',
            previous_values: JSON.stringify(kyc.dataValues),
  ...
  async function calculateKYCCompletion(kyc) {
    ...
    const optionalFields = ['driving_license_number', 'ration_card_number', 'arogyasri_card_number'];
    ...
    const mandatoryPercentage = (completedMandatory / mandatoryFields.length) * 70;
